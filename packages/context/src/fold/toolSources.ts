/**
 * Vendored from dsh-context `src/host/toolSources.ts` (Apache-2.0, see ../../NOTICE).
 *
 * Tool-to-plugin attribution for the Context browser's tool schema rows.
 *
 * A `request/header` event logs each tool as a plain schema
 * (`name`/`description`/`parameters` only), so the registering plugin is not
 * in the transcript. This module recovers what it can from the name alone:
 *
 * 1. A `plugin` field the raw header entry carries — passed through verbatim
 *    by headers.ts's `recordOf`, ahead of everything here.
 * 2. MCP tools: every proxied tool is named `mcp__<server>__<rawName>` (the
 *    convention dsh's MCP client, Claude Code and Codex all follow), so the
 *    server is recoverable from the name.
 * 3. The PINNED dsh first-party name → package map. Kept for completeness;
 *    {@link toolSourceOf} — the resolver this viewer installs — deliberately
 *    does NOT consult it, because a lowercase `read`/`bash` in a Claude or
 *    Codex transcript is not a dsh tool and must not be labelled as one.
 *
 * Pure string functions: fully unit-testable, no harness imports.
 */

/** MCP server-name prefix of proxied tools. */
...
/** The pinned package of a dsh first-party tool name, or undefined. */
export function pinnedSourceOf(name: string): string | undefined {
  return FIRST_PARTY_SOURCES[name]
}

/**
 * The attribution resolver this viewer installs on the headers view: MCP
 * servers only. The pinned first-party map names dsh packages, and neither
 * Claude Code nor Codex ships a tool by those names — consulting it could
 * only ever mislabel a same-named foreign tool.
 */
export function toolSourceOf(name: string): string | undefined {
  return mcpSourceOf(name)
}
