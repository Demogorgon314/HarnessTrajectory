/**
 * Vendored from dsh-context `src/host/logShapes.ts` (Apache-2.0, see ../../NOTICE).
 *
 * Shape-driven readers over the event vocabulary the fold consumes — the ONE
 * place it reconciles the two log generations a synthesizer may emit:
 *
 ...
 * yields "nothing here", never a throw (a throwing fold stalls the Context
 * tab on its loading state forever).
 */
...
/**
 * Whether an embedded assistant stream carries at least one usable
 * `block-start` marker (a finite time; the blockType may be unknown).
 *
 * PORT ADDITION — the fold needs this as a separate question from
 * {@link decodeSpansOfStream}: a stream whose single marker sits exactly at
 * the settlement instant tiles to all-zero spans yet still PROVES the model
 * window was framed. Harnesses that persist settled blocks rather than live
 * streams (Claude Code) emit exactly that shape with no token delta at all,
 * and the fold prices their generation window off this marker instead of a
 * first-token stamp it would have to fabricate.
 */
export function hasBlockStartMarker(stream: unknown): boolean {
  if (!Array.isArray(stream)) return false
  for (const record of stream) {
    if (record === null || typeof record !== 'object') continue
    const r = record as Record<string, unknown>
    if (r.type !== 'chunk' || r.chunk === null || typeof r.chunk !== 'object') continue
    if ((r.chunk as { type?: unknown }).type !== 'block-start') continue
    if (typeof r.time === 'number' && Number.isFinite(r.time)) return true
  }
  return false
}
