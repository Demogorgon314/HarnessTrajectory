/**
 * Vendored from dsh-context `src/shared/fileOps.ts` (Apache-2.0, see ../../NOTICE).
 *
 * The file-operation parser — the ONE derivation of "what the agent did to
 * files" from a settled file-tool call. The fold books the op log from the
 * tool lifecycle (tool/call + tool/result + tool/code-dispatch).
 *
 * NOTE for this port: the tool names below are the dsh built-ins. Claude Code
 * and Codex use different names, so their synthesizers precompute the ops and
 * hand them to the fold as `tool/result` `data.fileOps` ({@link FileOpInput});
 * this parser stays the FALLBACK for a result that carries none.
 *
 * Tool coverage matches the dsh built-ins: read / read_image / write / edit
 * (tool-fs) and grep / glob (tool-fs-search), plus the Anthropic-style
 * `str_replace_editor` (view reads, every other command writes). Line deltas
 * are estimates read off the
...
import type { FileOpRecord } from './types.ts'
 ...
 * fold's call/result pairing (args off the armed call, meta off the
 * result) and the nested Code-Mode settle (no meta exists on a dispatch — the
 * read window and per-file search attribution degrade to the argument-only
 * forms). Returns zero to
