/**
 * Wire contract of the Context dashboard. Vendored from dsh-context
 * (Apache-2.0, see ../../NOTICE); the dsh projection-registry augmentation
 * was removed — the timeline is produced locally by `ContextSession`.
 */

/**
 * Shared wire contract — the snapshot model exchanged between the fold and the
 * client. TYPE-ONLY module: both halves import these as `import type`.
 */
...
export interface Snapshot {
  ...
  /**
   * PORT ADDITION — `current.system` is a DERIVED remainder, not a measured
   * figure: the transcript never recorded the system prompt, so the fold
   * reconstructed it as `prompt − tools − messages` off the newest request's
   * provider-reported prompt size (clamped ≥ 0). The client labels such
   * figures "≈ derived". Absent when the harness recorded the prompt itself.
   */
  systemDerived?: true
  /**
   * PORT ADDITION — at least one request header carried a tool list, so
   * `current.tools` is a real (estimated) price. Absent means the harness
   * never recorded tool schemas: the client shows "not recorded" instead of
   * an honest-looking "0 items".
   */
  toolsKnown?: true
  current: {
    system: number
...
export interface RequestRecord {
  ...
  cacheRead?: number
  /**
   * PORT ADDITION — billed cache-WRITE prompt tokens of this request (the
   * other half of `prompt` alongside `cacheRead`). dsh read this figure from
   * the token-meter's own `tokenUsage` projection, which this viewer does not
   * have: the client sums the request records instead, so the fold must carry
   * it. Absent on usage-less requests; zero is a real value.
   */
  cacheWrite?: number
  output?: number
  /**
   * PORT ADDITION — this record's `system` figure is a derived remainder
   * (see {@link Snapshot.systemDerived}), not a price read off a recorded
   * prompt. `total` includes it.
   */
  systemDerived?: true
  /**
