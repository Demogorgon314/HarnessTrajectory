...
/**
 * dsh-context handed a row's path to the harness's browser-safe file-address
 * layer, which produced the `dsh-resource://file/…` address its right-Sidebar
 * preview type claimed. This viewer has no preview column, so the stub
 * answers undefined for every path and `previewAddressOf` reports "not
 * previewable" — the card then keeps its plain, inert file name. Replace this
 * with a real encoder to light the affordance up.
 */
function fileAddressFor(_sessionId: string, _workspace: string | undefined, _path: string): string | undefined {
  return undefined
}
