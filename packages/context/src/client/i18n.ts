...
export const DICT_ZH: Record<string, string> = {
  ...
  'stats.cost': '预估费用',
  'stats.costTip': '按 models.dev 刊例价估算的累计费用（≈ 估算），人民币按 1 元 = 0.15 美元换算。工具自己报告的金额（若有）才是权威值，可能高于也可能低于此估算：它既包含本会话日志未记录的调用（被打断的请求、后台生成标题的模型），也可能按协议价或厂商自己的计费价结算。',
  'stats.costTipDeepseek': 'DeepSeek 高峰时段（北京时间工作日 9:00–12:00、14:00–18:00）按挂牌价计费，其余时段半价。',
  'stats.costTipPeriod': '自定义价格模型按各自峰谷时段计费：峰时窗口按峰价，其余时段按谷价。',
  ...
  'stats.costPartial': '未计入 {n} 个模型（价目表中没有）：',
  'stats.costNoPriced': '{n} 个已计费模型均未定价：',
  'stats.costPriceAdd': '为它添加价格规则',
  'stats.costPriceEdit': '查看或修改它的价格规则',
  'stats.costUnavailable': '模型价格暂不可用（自动重试中），费用暂无法估算。',
  'stats.peakContext': '请求输入峰值',
  'stats.peakContextTip': '当前 Agent 已加载历史中，已记录的单次模型调用输入 token 最大值，包含缓存输入，不含本次输出、子 Agent 调用及压缩等辅助调用。≈ 表示估算，不能与实测混合。最高输入占用率按各调用当时记录的可用窗口计算，可能来自另一请求；窗口推断值不参与。覆盖率不足时不能代表完整会话峰值。',
  'stats.inputCoverage': '已记录输入：{n}/{total} 次调用',
  'stats.inputEstimate': '另有估算峰值：≈{n} tokens',
  'stats.inputRatio': '最高输入占用率：{n}%',
  'stats.windowCoverage': '窗口覆盖：{n}/{total} 次调用',
  'stats.duration': '总时长',
  'stats.durationTip': '从第一次请求到最后一条记录的自然时间。与「耗时统计」的活跃时长不同，这里包含等待、思考与空闲。',
  'stats.compactions': '压缩次数',
  'stats.compactionsTip': '本会话执行的上下文压缩次数。',
  'stats.compactionsPrunes': '另有 {n} 次裁剪。',
  'stats.subagents': '子 Agent',
  'stats.subagentsTip': '本会话派生的子 Agent 数量（不含主 Agent）。',
  'stats.costPerPrompt': '每次输入费用',
  'stats.costPerPromptTip': '预估费用除以用户输入次数。',
  'stats.images': '图片',
  'stats.imagesTip': '当前上下文中的图片块数量（用户上传与工具结果图片）。',
  'timing.title': '耗时统计',
  'timing.total': '活跃时长',
  'timing.ttft': '模型等待',
  'timing.gen': '模型生成',
  ...
  'session.costTip': '由该工具自己记录的累计费用（非估算），覆盖整个会话，包含所有子 Agent。它与上方「预估费用」高低皆有可能：本项覆盖会话日志未记录的调用，但按该厂商实际计费价结算，而估算只用 models.dev 刊例价。',
  'session.unknown': '未记录',
  'settings.open': '显示偏好',
  'derived.mark': '≈ 推导',
  ...
  'rich.md.diagram': '图表',
  'rich.md.source': '源码',
  'rich.md.diagramLoading': '正在渲染图表…',
  'rich.md.diagramError': '无法渲染图表，已显示源码。',
  'rich.md.expandDiagram': '放大图表',
  'rich.md.close': '关闭',
  'rich.md.zoom': '缩放',
  'rich.md.zoomIn': '放大',
  'rich.md.zoomOut': '缩小',
  'rich.md.resetZoom': '适应窗口',
  'rich.md.download': '下载 SVG',
  'block.thinking': '思考',
  'block.answer': '回答',
...
export const DICT_EN: Record<string, string> = {
  ...
  'stats.tipBadge': 'Explain this figure',
  ...
  'stats.cost': 'Cost',
  'stats.costTip': '≈ estimate at the models.dev list prices; the CNY display converts at 1 CNY = 0.15 USD. Where the harness reports its own figure, that one is authoritative and can land either side of this estimate: it also counts calls this transcript never records (interrupted requests, the background title model), and it may bill at negotiated or vendor-internal prices rather than list.',
  ...
  'stats.costTipPeriod': 'Custom-priced models bill by their own peak schedule: list price inside the peak windows, the off-peak rate outside.',
  ...
  'stats.costPartial': 'excludes {n} model(s) the price list does not carry: ',
  'stats.costNoPriced': 'none of the {n} billed model(s) is priced: ',
  'stats.costPriceAdd': 'Add a price rule for this model',
  'stats.costPriceEdit': 'View or edit this model\'s price rule',
  'stats.costUnavailable': 'Model prices are unavailable (retrying automatically) — the cost cannot be estimated yet.',
  'stats.peakContext': 'Peak Request Input',
  'stats.peakContextTip': 'Largest recorded input of one model call in this agent’s loaded history, including cached input. Excludes this response’s output, child-agent calls, and auxiliary calls such as compaction. ≈ marks separate estimates. Highest input occupancy uses each call’s recorded usable window and may come from a different request; inferred windows are excluded. Incomplete coverage is not a complete session peak.',
  'stats.inputCoverage': 'Recorded input: {n}/{total} calls',
  'stats.inputEstimate': 'Separate estimated peak: ≈{n} tokens',
  'stats.inputRatio': 'Highest input occupancy: {n}%',
  'stats.windowCoverage': 'Window coverage: {n}/{total} calls',
  'stats.duration': 'Duration',
  'stats.durationTip': 'Wall-clock time from the first request to the last record. Unlike Timing Stats’ active time, this includes waiting, thinking and idle gaps.',
  'stats.compactions': 'Compactions',
  'stats.compactionsTip': 'Context compactions the session performed.',
  'stats.compactionsPrunes': '+{n} prunes.',
  'stats.subagents': 'Subagents',
  'stats.subagentsTip': 'Child agents of this session (the main agent excluded).',
  'stats.costPerPrompt': 'Cost / Prompt',
  'stats.costPerPromptTip': 'Estimated cost divided by human prompts.',
  'stats.images': 'Images',
  'stats.imagesTip': 'Image blocks live in the current context (user uploads and tool-result images).',
  ...
  'session.costTip': 'The cumulative cost the harness recorded itself — measured, not estimated — for the whole session, subagents included. It can land either side of the Context Stats estimate: it covers calls this transcript never records, but it bills at the vendor’s own prices while the estimate only knows the models.dev list.',
  ...
  'rich.md.diagram': 'Diagram',
  'rich.md.source': 'Source',
  'rich.md.diagramLoading': 'Rendering diagram…',
  'rich.md.diagramError': 'Unable to render diagram. Source shown below.',
  'rich.md.expandDiagram': 'Expand diagram',
  'rich.md.close': 'Close',
  'rich.md.zoom': 'Zoom',
  'rich.md.zoomIn': 'Zoom in',
  'rich.md.zoomOut': 'Zoom out',
  'rich.md.resetZoom': 'Fit to view',
  'rich.md.download': 'Download SVG',
