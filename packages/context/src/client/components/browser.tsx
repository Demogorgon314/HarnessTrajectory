...
import type { ContentFetcher, ConversationNodeLike, DetailState, HeaderFetcher } from '../services'
...
export interface ContextBrowserProps {
   ...
   * exactly one place.
   */
  convNodes?: readonly ConversationNodeLike[] | undefined
  /**
   ...
   */
  fetchContent?: ContentFetcher | undefined
  /**
   ...
   */
  nodeFocus?: { step: number | 'live'; seq: number; cat: Category } | null | undefined
  onNodeFocusHandled?: (() => void) | undefined
  hoverKey?: string | null | undefined
  onHoverKey?: ((key: string | null) => void) | undefined
  /**
   ...
   */
  onOpenCat?: ((cat: string | null) => void) | undefined
  ...
  onDetailRetry?: (() => void) | undefined
  /**
   * PORT ADDITION — the System Prompt figure is a DERIVED remainder: this
   * harness's transcript never recorded the prompt, so the row carries the
   * `≈ derived` marker and the open section explains the gap instead of
   * showing a prompt that does not exist.
   */
  systemDerived?: boolean | undefined
  /**
   * PORT ADDITION — at least one request header carried a tool list. False
   * means the harness records no tool schemas at all, so the Tool Schemas row
   * says "not recorded" rather than claiming 0 items.
   */
  toolsKnown?: boolean | undefined
}
...
export function makeContextBrowser(
  ...
  return function ContextBrowser(props: ContextBrowserProps): ReactElement {
    ...
    const toggleCat = (c: string) => {
      // Empty cats stay shut — except system/tools with no header epoch (or an
      // unrecorded/derived figure), which open to explain the degradation note.
      const openable = toolCount(c) > 0
        || ((c === 'system' || c === 'tools') && view.header === null)
        || (c === 'tools' && props.toolsKnown === false)
        || (c === 'system' && props.systemDerived === true)
      if (!openable) return
      if (openCat === c) {
        setCat(null)
        setOpenElem(null)
    ...
    const catBody = (c: string): ReactNode => {
      if (c === 'system') {
        ...
        if (sys === null) {
          // PORT ADDITION — a derived remainder has no prompt text to show at
          // all; say so instead of blaming retention.
          if (props.systemDerived === true) return <div className="lc-br-note">{t('derived.systemBody')}</div>
      ...
      if (c === 'tools') {
        // PORT ADDITION — the harness records no schemas: that is a property of
        // the log, not a retention miss.
        if (props.toolsKnown === false) return <div className="lc-br-note">{t('tools.unrecordedBody')}</div>
      ...
      <div className="lc-card" ref={rootRef}>
        ...
        <div className="lc-br-cats">
          {CATS.map((c) => {
            ...
            // PORT ADDITION — the two honesty arms open on their own so the
            // section can explain what the harness did not record.
            const unrecordedTools = c.key === 'tools' && props.toolsKnown === false
            const derivedSystem = c.key === 'system' && props.systemDerived === true
            const openable = count > 0
              || ((c.key === 'system' || c.key === 'tools') && view.header === null)
              || unrecordedTools || derivedSystem
            ...
            return (
              <div key={c.key} className={'lc-br-cat' + (openable ? '' : ' lc-br-cat-empty')}>
                ...
                >
                  ...
                  {derivedSystem
                    ? <span className="lc-br-cat-note" title={t('derived.systemTip')}>{t('derived.mark')}</span>
                    : null}
                  ...
                  <span className="lc-br-count-grp">
                    <span className="lc-br-cat-count">
                      {unrecordedTools ? t('tools.unrecorded') : t('browser.items', { n: count })}
                    </span>
