...
export interface RequestDetailProps {
   ...
   * deltaOf plots.
   */
  prev?: RequestRecord | null | undefined
  marker?: ContextEventRecord | null | undefined
   ...
   */
  stepsOf?: ((turn: number | undefined) => number) | undefined
  /** Reveal a brief row's node in the Context browser; absent = rows render inert. */
  onLocate?: ((node: SurfaceNode, isResponse: boolean) => void) | undefined
  /**
...
export function makeRequestDetail(
  ...
  function BriefSection(props: {
    brief: StepBrief | null | undefined
    convOf?: ((seq: number) => ConversationNodeLike | undefined) | undefined
    onLocate?: ((node: SurfaceNode, isResponse: boolean) => void) | undefined
  }): ReactElement {
  ...
  return memo(function RequestDetail(props: RequestDetailProps): ReactElement | null {
    ...
    const parts = delta
      ? CATS.map((c, i) => ({ key: c.key, color: c.color, value: Math.abs(deltas[i] ?? 0) }))
      : partsOf(req)
    return (
      <div className="lc-detail">
        ...
        <div className="lc-detail-rows">
          {CATS.map((c, i) => {
            const v = delta ? (deltas[i] ?? 0) : req[c.key] || 0
