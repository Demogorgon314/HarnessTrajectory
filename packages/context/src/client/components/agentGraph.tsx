/**
 * The Agent network card — the foot of the Context tab: this session's whole
 * agent family (the main agent and every subagent transcript) as a node
 * graph, where every node is a live donut of that agent's own context
 * composition ringed by its occupancy, and a click opens that agent's own
 * Context view.
 *
 * dsh-context read the family off the harness session list; here the caller
 * hands in one row per transcript file (`AgentNodeInput`) and one navigation
 * callback, so the card owns no service lookup of its own.
 *
...
import type { AgentNode, AgentNodeInput } from '../agentTree'
import {
  AGENT_NODE_R,
  AGENT_RING_R,
  agentForestOf,
...
export interface AgentGraphProps {
  /** Every agent of this session; the caller folds each transcript file. */
  agents: readonly AgentNodeInput[]
  /** The agent whose context the view is showing. */
  currentId: string
...
export function makeAgentGraph(
  ...
  function AgentGraph(props: AgentGraphProps): ReactElement | null {
    ...
    const built = useMemo(() => {
      const forest = agentForestOf(agents, currentId)
      return forest !== null ? { forest, layout: layoutForest(forest, stageWidth) } : null
    }, [agents, currentId, stageWidth])
...
function Inspector(props: { node: AgentNode; t: ViewKit['t']; fmt: ViewKit['fmt'] }): ReactElement {
  ...
  return (
    <div className="lc-agents-inspector">
      ...
      {node.badge !== null ? <span className="lc-agents-badge">{node.badge}</span> : null}
