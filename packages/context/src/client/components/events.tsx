/**
  * Glyphs: inject/model-switch reuse the shared icon set (`@harness-trajectory/ui`);
...
import type { DetailState } from '../services'
import type { ViewKit } from '../viewkit'
import { makeDetailNote } from './detailNote'

const { IconBranchOutline16, IconPlusOutline16 } = icons
...
export function makeEventText(t: Translate): {
  ...
  function eventLabel(ev: ContextEventRecord): string {
    ...
    // A producer name the dictionary knows (e.g. the compaction-retained
    // history a Codex compaction re-injects) reads as its label; any other
    // name stays verbatim, as dsh renders plugin ids and file paths.
    const named = ev.name ? t('ev.name.' + ev.name) : ''
    const name = ev.name && named !== 'ev.name.' + ev.name ? named : ev.name
    let label = name ? base + ' · ' + name : base
