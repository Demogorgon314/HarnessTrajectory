...
export interface StackedBarProps {
  ...
  onPickKey?: ((key: string) => void) | undefined
  /**
   ...
   */
  reserve?: { ratio: number; label: string } | undefined
}
...
export interface LegendProps {
  ...
  onHoverKey?: ((key: string | null) => void) | undefined
  /**
   * PORT ADDITION — the part whose figure is a DERIVED remainder rather than
   * a measured price (a harness that never records its system prompt): the
   * chip carries the `≈ derived` marker and explains itself on hover.
   */
  derivedKey?: string | null | undefined
}

export function makeLegend(kit: ViewKit): (props: LegendProps) => ReactElement {
  const { t, fmt, catLabel } = kit
  return function Legend(props: LegendProps): ReactElement {
    let total = 0
    ...
    return (
      <div className="lc-legend">
        {props.parts.map((p) => {
          ...
          const derived = props.derivedKey !== undefined && props.derivedKey !== null && props.derivedKey === p.key
          return (
            <span
              key={p.key}
              className={'lc-chip' + (on ? ' lc-chip-on' : '') + (derived ? ' lc-chip-derived' : '')}
              title={derived ? t('derived.systemTip') : t('overview.ofUsed')}
              onMouseEnter={() => { if (props.onHoverKey !== undefined) props.onHoverKey(p.key) }}
              onMouseLeave={() => { if (props.onHoverKey !== undefined) props.onHoverKey(null) }}
            >
              <i style={{ background: p.color }} />
              <span className="lc-chip-label">{catLabel(p.key)}</span>
              {derived ? <span className="lc-chip-note">{t('derived.mark')}</span> : null}
