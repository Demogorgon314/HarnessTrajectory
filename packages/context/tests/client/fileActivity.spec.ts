...
/** `Partial` with explicit `undefined` allowed: this repo compiles with `exactOptionalPropertyTypes`. */
...
function op(
  ...
  extra: Loose<SurfaceNode> = {},
  convExtra: Partial<ConversationNodeLike> = {},
): { node: SurfaceNode; conv: ConversationNodeLike } {
  return {
    node: { seq, cat: 'tool', tokens: 5, tool, time: T0 + seq, ...extra } as SurfaceNode,
    conv: {
      kind: 'tool',
      seq,
...
describe('previewAddressOf', () => {
  // PORT NOTE — `fileAddressFor` is stubbed in this port (this viewer has no
  // sidebar preview column), so EVERY path answers "not previewable" and the
  // card keeps its inert file name. The guard arms below still hold, and are
  // what a real encoder would have to keep honoring.
  test('no path is previewable while the address encoder is stubbed', () => {
    const ws = '/repo'
    assert.equal(previewAddressOf('src/a.ts', 'text', undefined, 's1', ws), undefined)
    assert.equal(previewAddressOf('/repo/src/a.ts', 'text', undefined, 's1', ws), undefined)
    assert.equal(previewAddressOf('rel/b.md', 'text', undefined, 's1', undefined), undefined)
    assert.equal(previewAddressOf('/shots/ui.png', 'image', undefined, 's1', ws), undefined)
  })
