// Client test kit: the real dictionaries, the real locale chain, and real
// React rendering in jsdom. Shared by every client spec so each test file
// stays small and stateless.
//
// PORT NOTE — dsh-context paired this with `helpers/harness.ts`, a faithful
// in-memory implementation of the harness client context (cordis
// effect/inject, the slot registry, the sessions scope), because the plugin's
// `apply` consumed all of it. This port has no plugin entry — every view takes
// props — so only the locale chain survives, and it lives here.
