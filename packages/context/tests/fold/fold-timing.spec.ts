...
import { assertPlainJson, assertStable, driveTimeline, timelineDef } from './helpers/drive.ts'
...
describe('timing — the generation split (reasoning / text / tool args)', () => {
  ...
  // PORT DEVIATION (see the `assistant/message` timing branch in fold.ts):
  // a harness that persists SETTLED blocks — Claude Code writes one record per
  // completed content block — can never report a first-token instant, so its
  // synthesizer frames the window with `block-start` markers alone rather than
  // fabricating a TTFT. dsh left such a call unattributed wholesale; here the
  // whole model window counts as generation and the markers still tile it.
  describe('a block-framed stream with no token delta', () => {
    /** The Claude shape: block i opens when block i-1 completed, block 0 at step start. */
    const blockFramed = (times: [number, 'reasoning' | 'text' | 'tool-call'][]): unknown[] =>
      times.map(([time, blockType]) => ({ type: 'chunk', time, chunk: { type: 'block-start', blockType } }))

    test('the whole step → settlement span counts as generation, with TTFT unattributed', () => {
      const { state } = driveTimeline([
        stepStart(1, { time: 1_000 }),
        assistantMessage(2, { time: 5_000, stream: blockFramed([[1_000, 'text']]) }),
      ])
      assert.equal(state.timing?.ttftMs, 0, 'no first token was recorded — the wait stays unattributed')
      assert.equal(state.timing?.genMs, 4_000, 'the model window is generation')
      assert.equal(state.timing?.calls, 1)
    })

    test('the markers tile that window into reasoning / text / tool args', () => {
      const { state } = driveTimeline([
        stepStart(1, { time: 0 }),
        assistantMessage(2, {
          time: 2_000,
          stream: blockFramed([[0, 'reasoning'], [700, 'text'], [1_200, 'tool-call']]),
        }),
      ])
      const t = state.timing
      assert.equal(t?.genMs, 2_000)
      assert.equal(t?.reasoningMs, 700, 'reasoning owns 0→700')
      assert.equal(t?.textMs, 500, 'text owns 700→1200')
      assert.equal(t?.toolArgMs, 800, 'tool args own 1200→settlement')
      assert.equal((t?.reasoningMs ?? 0) + (t?.textMs ?? 0) + (t?.toolArgMs ?? 0), t?.genMs, 'the split tiles the window exactly')
      assert.equal(t?.ttftMs, 0)
    })

    test('a single marker at the settlement instant still prices the window', () => {
      // The spans tile to zero (the marker IS the end), but the window is real.
      const { state } = driveTimeline([
        stepStart(1, { time: 0 }),
        assistantMessage(2, { time: 900, stream: blockFramed([[900, 'text']]) }),
      ])
      assert.equal(state.timing?.genMs, 900)
      assert.equal(state.timing?.textMs, undefined, 'a zero span stays absent')
    })

    test('an unknown blockType frames the window without poisoning a bucket', () => {
      const { state } = driveTimeline([
        stepStart(1, { time: 0 }),
        assistantMessage(2, {
          time: 1_000,
          stream: [{ type: 'chunk', time: 200, chunk: { type: 'block-start', blockType: 'image' } }],
        }),
      ])
      assert.equal(state.timing?.genMs, 1_000)
      assert.equal(state.timing?.reasoningMs, undefined)
    ...
    test('the totals accumulate across steps and stay plain JSON', () => {
      const drive = driveTimeline([
        stepStart(1, { time: 0 }),
        assistantMessage(2, { time: 1_000, stream: blockFramed([[0, 'text']]) }),
        stepEnd(3, { time: 1_100 }),
        stepStart(4, { time: 10_000 }),
        assistantMessage(5, { time: 10_500, stream: blockFramed([[10_000, 'reasoning']]) }),
        stepEnd(6, { time: 10_600 }),
      ])
      assert.equal(drive.state.timing?.genMs, 1_500)
      assert.equal(drive.state.timing?.textMs, 1_000)
      assert.equal(drive.state.timing?.reasoningMs, 500)
      assert.equal(drive.state.timing?.ttftMs, 0)
      assertPlainJson(drive.state)
    })

    test('a token-bearing stream keeps the dsh figures (the deviation never applies)', () => {
      const { state } = driveTimeline([
        stepStart(1, { time: 0 }),
        assistantMessage(2, {
          time: 2_000,
          stream: [
            { type: 'chunk', time: 200, chunk: { type: 'block-start', blockType: 'reasoning' } },
            { type: 'chunk', time: 300, chunk: { type: 'text-delta', text: 'x' } },
            { type: 'chunk', time: 700, chunk: { type: 'block-start', blockType: 'text' } },
          ],
        }),
      ])
      assert.equal(state.timing?.ttftMs, 300, 'the recorded first token still anchors the wait')
      assert.equal(state.timing?.genMs, 1_700, 'generation opens at the token, not at the step')
      assert.equal(state.timing?.reasoningMs, 500)
      assert.equal(state.timing?.textMs, 1_300)
    })

    test('a stream with no block marker at all stays unattributed', () => {
      for (const stream of [undefined, [], 'nope', [null, 7], [{ type: 'chunk', time: 5, chunk: { type: 'finish' } }],
    ...
    test('a settlement with no open step prices nothing, marker or not', () => {
      const { state } = driveTimeline([
        assistantMessage(1, { time: 900, stream: blockFramed([[100, 'text']]) }),
      ])
      assert.equal(state.timing?.genMs, 0)
      assert.equal(state.timing?.calls, 1)
    })
...
describe('timing — served wire view', () => {
  ...
  test('every timing-bearing state stays losslessly serializable (stamped slot included)', () => {
    // dsh gated this with the projection cache's zod state schema; this port
    // keeps the precondition the gate stood for — a fold state must survive a
    // structuredClone / JSON round trip with no undefined-valued property.
    const drive = driveTimeline([...step(1, 0, 1_000, { tokenMs: 300 }),
      toolCall(5, { callId: 'c1', name: 'bash' }),
      toolResult(6, { callId: 'c1', content: text('ok') }),
      stepEnd(7, { time: 30_000 })])
    for (const state of drive.states) {
      assert.deepEqual(assertPlainJson(state), structuredClone(state))
    }
  })
})
