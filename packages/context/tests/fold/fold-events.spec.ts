...
describe('hostile provider usage (issue #44: stats must survive nonconforming figures)', () => {
  ...
  test('the folded state and served view stay well-shaped under hostile usage', () => {
    ...
    // dsh gated this with the registry's integer schemas; here we pin what
    // they stood for — every billed figure is a non-negative safe integer and
    // the whole state/view stays losslessly serializable.
    assertPlainJson(drive.state)
    assertPlainJson(drive.def.view(drive.state))
    for (const record of drive.state.requests) {
      for (const figure of [record.prompt, record.cacheRead, record.cacheWrite, record.output]) {
        if (figure !== undefined) assert.ok(Number.isSafeInteger(figure) && figure >= 0, String(figure))
      }
    }
  })
...
describe('hostile events', () => {
  test('an assistant/message without a message payload never takes the fold down', () => {
    // PORT DEVIATION: dsh's `deriveEventMessage` dereferenced
    // data.message.content and THREW here, so the event was dropped whole by
    // apply's catch. The local reader (src/fold/fold.ts) returns "no message"
    // instead — a usage-only settlement is a real shape — so the request
    // record lands with a zero-token node. Either way nothing throws out of
    // apply, which is what this case has always been about.
    const def = timelineDef({})
    const init = def.init()
    const next = def.apply(init, { type: 'assistant/message', seq: 1, time: at(), data: {} } as TimelineEvent)
    assert.equal(next.surface.length, 1)
    assert.equal(next.surface[0]!.tokens, 0)
    ...
    assert.equal(then.surface.length, 2)
    assert.equal(then.requests.length, 2)
