// The fold-derived file-op log (src/fold/fold.ts + shared/fileOps.ts): ops
...
// copies. Driven through the real fold.

import assert from './helpers/assert.ts'
import { describe, test } from 'vitest'
import type { TimelineEvent } from '../../src/fold/fold.ts'
import { buildTimelineDetail, buildTimelineHead, buildTimelineView } from '../../src/fold/fold.ts'
import { resolveBounds } from '../../src/fold/config.ts'
import { codeDispatch, toolCall, toolResult } from './helpers/events.ts'
import { assertPlainJson, assertStable, assertStatesPlainJson, driveTimeline, row } from './helpers/drive.ts'
...
// PORT ADDITION — `tool/ops`: a LATE file-op booking for a `tool/result` that
// already folded. Codex writes a share of its `item_completed` records after
// the tool output line, so its synthesizer emits this follow-up instead of
// deferring every result on the chance that a late record arrives.
describe('the file-op log — late bookings (tool/ops)', () => {
  const lateOps = (seq: number, time: number, data: Record<string, unknown>): TimelineEvent =>
    ({ type: 'tool/ops', seq, time, data })

  /** A settled Bash-ish call whose result folds at seq 2 with no ops of its own. */
  const settled = (): TimelineEvent[] => [
    toolCall(1, { callId: 'c1', name: 'exec', arguments: JSON.stringify({ command: 'sed -n 1,40p a.ts' }) }),
    toolResult(2, { callId: 'c1', content: [{ type: 'text', text: 'ok' }], time: 2_000 }),
  ]

  test('the ops land under the RESULT\'s seq with the late event\'s time', () => {
    const { state } = driveTimeline([
      ...settled(),
      lateOps(3, 9_000, { resultSeq: 2, fileOps: [{ kind: 'read', path: 'a.ts', read: { start: 1, count: 40 } }] }),
    ])
    assert.deepEqual(state.fileOps, [{
      seq: 2, time: 9_000, tool: 'exec', err: false,
      kind: 'read', path: 'a.ts', added: 0, removed: 0, read: { start: 1, count: 40 },
    }])
  })

  test('the tool name comes from the event, else from what the result left behind', () => {
    const named = driveTimeline([
      ...settled(),
      lateOps(3, 3_000, { resultSeq: 2, tool: 'shell', fileOps: [{ kind: 'write', path: 'a.ts', added: 2 }] }),
    ])
    assert.equal(row(named.state.fileOps, 0).tool, 'shell', 'an explicit name wins')

    const recovered = driveTimeline([
      ...settled(),
      lateOps(3, 3_000, { resultSeq: 2, fileOps: [{ kind: 'write', path: 'a.ts' }] }),
    ])
    assert.equal(row(recovered.state.fileOps, 0).tool, 'exec', 'recovered off the surface node the result left')

    const unknown = driveTimeline([
      lateOps(1, 3_000, { resultSeq: 99, fileOps: [{ kind: 'write', path: 'a.ts' }] }),
    ])
    assert.equal(row(unknown.state.fileOps, 0).tool, '', 'nothing knows it — the same honest blank an unpaired result books')
  })

  test('a late booking splices into seq order instead of appending out of order', () => {
    const { state } = driveTimeline([
      toolCall(1, { callId: 'c1', name: 'exec' }),
      toolResult(2, { callId: 'c1', content: [{ type: 'text', text: 'ok' }] }),
      toolCall(3, { callId: 'c2', name: 'apply_patch' }),
      toolResult(4, { callId: 'c2', content: [{ type: 'text', text: 'ok' }], fileOps: [{ kind: 'write', path: 'late.ts', added: 1 }] }),
      // The record for the OLDER result arrives now.
      lateOps(5, 5_000, { resultSeq: 2, fileOps: [{ kind: 'read', path: 'early.ts' }] }),
    ])
    assert.deepEqual(state.fileOps.map(o => [o.seq, o.path]), [[2, 'early.ts'], [4, 'late.ts']])
  })

  test('a batch newer than the whole log simply appends', () => {
    const { state } = driveTimeline([
      toolCall(1, { callId: 'c1', name: 'exec' }),
      toolResult(2, { callId: 'c1', content: [{ type: 'text', text: 'ok' }], fileOps: [{ kind: 'read', path: 'a.ts' }] }),
      lateOps(5, 5_000, { resultSeq: 4, fileOps: [{ kind: 'read', path: 'b.ts' }] }),
    ])
    assert.deepEqual(state.fileOps.map(o => [o.seq, o.path]), [[2, 'a.ts'], [4, 'b.ts']])
  })

  test('several ops of one batch keep their stated order at the same seq', () => {
    const { state } = driveTimeline([
      ...settled(),
      lateOps(3, 3_000, {
        resultSeq: 2,
        fileOps: [
          { kind: 'write', path: 'a.ts', added: 3, removed: 1 },
          { kind: 'write', path: 'b.ts', added: 0, removed: 7 },
          { kind: 'search', path: 'TODO', pattern: true, detail: 'TODO' },
        ],
      }),
    ])
    assert.deepEqual(state.fileOps.map(o => [o.path, o.added, o.removed]), [
      ['a.ts', 3, 1], ['b.ts', 0, 7], ['TODO', 0, 0],
    ])
    assert.ok(state.fileOps.every(o => o.seq === 2))
    assert.equal(row(state.fileOps, 2).pattern, true)
  })

  test('the error flag rides the event, and a row may raise it on its own', () => {
    const { state } = driveTimeline([
      ...settled(),
      lateOps(3, 3_000, { resultSeq: 2, err: true, fileOps: [{ kind: 'write', path: 'a.ts' }] }),
      lateOps(4, 4_000, { resultSeq: 2, fileOps: [{ kind: 'write', path: 'b.ts', err: true }, { kind: 'write', path: 'c.ts' }] }),
    ])
    assert.deepEqual(state.fileOps.map(o => [o.path, o.err]), [
      ['a.ts', true], ['b.ts', true], ['c.ts', false],
    ])
  })

  test('a resultSeq already below the coverage floor is dropped', () => {
    ...
    const { def, state } = driveTimeline(events, { maxFileOps: 3 })
    assert.equal(state.fileOpsFloor, 13)
    // seq 13 and below were trimmed: a late row for them covers a window the
    // log no longer represents.
    for (const resultSeq of [11, 13]) {
      assert.equal(
        def.apply(state, lateOps(30, 30_000, { resultSeq, fileOps: [{ kind: 'read', path: 'gone.ts' }] })),
        state,
        `resultSeq ${resultSeq} is at or below the floor`,
      )
    }
    // Just above the floor still books.
    const kept = def.apply(state, lateOps(30, 30_000, { resultSeq: 15, fileOps: [{ kind: 'read', path: 'kept.ts' }] }))
    assert.notEqual(kept, state)
    assert.ok(kept.fileOps.some(o => o.path === 'kept.ts'))
  })

  test('malformed or empty payloads return the SAME state reference', () => {
    const { state } = driveTimeline(settled())
    for (const data of [
      undefined,
      {},
      { fileOps: [{ kind: 'read', path: 'a.ts' }] }, // no resultSeq
      { resultSeq: '2', fileOps: [{ kind: 'read', path: 'a.ts' }] },
      { resultSeq: Number.NaN, fileOps: [{ kind: 'read', path: 'a.ts' }] },
      { resultSeq: 2 }, // no ops
      { resultSeq: 2, fileOps: [] },
      { resultSeq: 2, fileOps: 'nope' },
      { resultSeq: 2, fileOps: [null, 7, { kind: 'read' }, { kind: 'sideways', path: 'a.ts' }] }, // every row junk
    ]) {
      assertStable(state, { type: 'tool/ops', seq: 9, time: 9_000, ...(data === undefined ? {} : { data }) })
    }
  })

  test('the ops reach the served view and bump the detail revision', () => {
    const before = driveTimeline(settled())
    const { def } = before
    const after = def.apply(before.state, lateOps(3, 3_000, { resultSeq: 2, fileOps: [{ kind: 'read', path: 'a.ts' }] }))
    assert.equal(after.detailRev, (before.state.detailRev ?? 0) + 1)
    const view = buildTimelineView(after, resolveBounds({}))
    assert.equal(view.fileOps?.length, 1)
    assert.equal(view.fileOps?.[0]?.seq, 2)
    assert.notEqual(view.fileOps?.[0]!, row(after.fileOps, 0), 'the served op never aliases the state')
    assert.equal(buildTimelineDetail(after, resolveBounds({})).fileOps?.length, 1)
    assertPlainJson(after)
  })

  test('late bookings still trim under maxFileOps', () => {
    const { state } = driveTimeline([
      ...settled(),
      lateOps(3, 3_000, {
        resultSeq: 2,
    ...
    assert.deepEqual(state.fileOps.map(o => o.path), ['b.ts', 'c.ts'])
    assert.equal(state.fileOpsFloor, 2)
  })
})
...
describe('the file-op log — retention, schema, and the served payload', () => {
  ...
  test('a state carrying ops stays serializable; the detail builder serves detached copies', () => {
    const { state } = driveTimeline([
      toolCall(1, { callId: 'c1', name: 'read', arguments: JSON.stringify({ file_path: 'a.ts' }) }),
    ...
    assert.notEqual(detail.fileOps?.[0]!, row(state.fileOps, 0), 'the served op never aliases the state')
  ...
  test('the slim head does NOT carry the op log; the inline value does', () => {
    ...
    assert.equal('fileOps' in buildTimelineHead(state), false)
    const full = buildTimelineView(state, resolveBounds({}))
