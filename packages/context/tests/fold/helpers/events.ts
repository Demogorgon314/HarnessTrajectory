// Event envelope builders for the fold specs. Shapes mirror the synthesizer→
// fold contract (`src/fold/event.ts`) — the same envelopes a real synthesizer
// emits, minus fields the fold never reads.
//
// Vendored from dsh-context `tests/host/helpers/events.ts` (Apache-2.0).

import type { ContentBlock, MessageSource, TimelineEvent } from '../../../src/fold/event.ts'
...
/** user/message: the payload IS the message (deriveEventMessage returns data). */
...
export function toolResult(seq: number, opts: {
  ...
  error?: boolean
  /** Drop the source (a legacy/foreign envelope). */
  noSource?: boolean
  /** Drop the envelope callId (source carries it). */
  noEnvelopeId?: boolean
  /** The bounded presentation meta (search matches / read window). */
  meta?: unknown
...
/** assistant/message: the payload nests the message under `data.message`. */
...
/** A nested Code-Mode dispatch settling inside a run_code program — both vocabulary generations. */
export function codeDispatch(seq: number, opts: {
  ...
  isError?: unknown
  time?: number
  /** The newer vocabulary tag (`tool/ptc-dispatch`); defaults to `tool/code-dispatch`. */
  type?: string
}): TimelineEvent {
  return {
