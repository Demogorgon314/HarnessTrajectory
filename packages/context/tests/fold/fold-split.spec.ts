// The two view generations (src/fold/fold.ts buildTimelineHead /
...
import { assertPlainJson, driveTimeline, row, timelineDef } from './helpers/drive.ts'
...
describe('the two view generations', () => {
  test('the slim head carries the markers and no collections', () => {
    const { state } = driveTimeline(canonicalLog())
    const view = buildTimelineHead(state)
    assert.equal(typeof view.detailRev, 'number', 'the split marker is present')
    assert.ok(view.counts !== undefined)
    assert.deepEqual(view.nodes, [], 'collections stay off the head')
    assertPlainJson(view)
  })

  test('the inline view carries the collections and NO split markers', () => {
    const { state } = driveTimeline(canonicalLog())
    const view = buildTimelineView(state, resolveBounds({}))
  ...
  test('equivalence: the inline view is exactly the head plus the detail (minus the markers)', () => {
    ...
    const head = buildTimelineHead(state)
    const detail = buildTimelineDetail(state, resolveBounds({}))
    const inline = buildTimelineView(state, resolveBounds({}))
