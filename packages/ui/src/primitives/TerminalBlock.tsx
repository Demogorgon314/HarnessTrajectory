...
export interface TerminalBlockProps {
  /** Viewer override: historical transcripts may omit settlement or report a
   * tool-level error without a shell exit code. Never fabricate that code. */
  recordedState?: { state: StateDotState; label: string } | undefined
...
export function TerminalBlock({
  ...
  const state = recordedState ?? runState(running, exitCode, signal, copy)
  ...
  const body = (!running && recordedState?.state !== 'idle') || !empty
