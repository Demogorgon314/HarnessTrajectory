...
import type { ChatTurnItem } from './turn-rail.ts'
import css from './TurnNavigator.module.css'

interface TurnNavigatorProps {
  readonly items: readonly ChatTurnItem[]
  readonly activeTurn: number | null
  readonly onNavigate: (item: ChatTurnItem) => void
...
function frameStyle(count: number, scrollTop: number, band: number): TurnFrameStyle {
  return {
    '--turn-rail-band': `${String(band)}px`,
...
function itemAtPointer(
  items: readonly ChatTurnItem[],
  frame: HTMLElement,
  scrollTop: number,
  clientY: number,
): ChatTurnItem | undefined {
...
function TurnNavigatorRail({ items, activeTurn, onNavigate, labels }: TurnNavigatorProps) {
  ...
  // The slot unmounts when the rail hides, so it is state: mounting re-measures.
  const [slot, setSlot] = useState<HTMLDivElement | null>(null)
  const [band, setBand] = useState(0)
  const [viewportHeight, setViewportHeight] = useState(0)
  const followedHeight = useRef(0)
  ...
  // The slot's parent is the scrollport; its height is the band the rail
  // floats in. Measure it here so no host has to publish a viewport var.
  useLayoutEffect(() => {
    const scrollport = slot?.parentElement ?? null
    if (scrollport === null) return
    const measure = (): void => { setBand(scrollport.clientHeight) }
    measure()
  ...
  // Observe the actual viewport, including intermediate CSS transition sizes.
  // Band changes alone don't describe the clipped area the marks can occupy.
  useEffect(() => {
    const scroller = scrollerRef.current
    if (scroller === null) return
    const measure = (): void => {
  ...
  useEffect(syncScrollState, [items.length, band])

  // Keep the active mark visible: centre it whenever it leaves the scrollport,
  // unless the reader's pointer is working the rail.
  useEffect(() => {
    const resized = followedHeight.current !== viewportHeight
    followedHeight.current = viewportHeight
    const scroller = scrollerRef.current
    ...
    if (typeof scroller.scrollTo === 'function') {
      // Resizing already animates the viewport; don't restart a smooth scroll
      // on every transition frame and leave the active mark chasing its edge.
      scroller.scrollTo({ top: target, behavior: resized || reduced ? 'instant' : 'smooth' })
    } else {
      scroller.scrollTop = target
    }
    syncScrollState()
  }, [activeTurn, items, viewportHeight])
  ...
  return (
    <div ref={setSlot} className={css.slot}>
      <nav
        className={css.frame}
        style={frameStyle(items.length, scrollState.top, band)}
        ...
        >
          <div className={css.marks}>
            {items.map((item, index) => {
              ...
              if (!item.loaded) classes.push(css.markUnloaded)
              ...
              return (
                <div key={item.turn} className={css.markPosition} style={itemPosition(index)}>
                  <button
                    ...
                    aria-label={item.loaded ? labels.jumpToTurn(item.turn) : labels.jumpToTurnLoad(item.turn)}
        ...
        {preview !== undefined && previewPosition !== undefined && (
          <div id={previewId} role="tooltip" className={css.preview} style={previewPosition}>
            <div className={css.previewPrompt}>
              {preview.prompt || labels.turnLabel(preview.turn)}
            </div>
...
/**
 * Fixed-pitch rail of every turn the snapshot knows — loaded marks scroll,
 * unloaded marks page the window back first — with hover and focus previews.
 * Overflow scrolls inside the frame, gradient fades marking each scrollable
 * end, and the active mark keeps itself in view while the pointer is
 * elsewhere.
 *
 ...
 * turn is added or becomes active. Its props must stay referentially stable.
 */
