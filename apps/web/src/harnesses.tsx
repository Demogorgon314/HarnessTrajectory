 ...
 * underscore, not the project's own logo. The Cursor mark is an in-house
 * pointer monogram, not the project's cube. The dsh mark is DeepSeek's fish
...
/** Cursor mark: an in-house monogram — a pointer, drawn in `currentColor`. */
export function CursorLogo({ size = 16, className, style }: HarnessLogoProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className={className} style={style} aria-hidden="true">
      <path d="M5.2 3.2 19.4 12.1 12.2 13.6 10.4 20.6 5.2 3.2z" fill="currentColor" />
      <path d="M12.2 13.6 14.6 19.2" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" opacity=".55" />
    </svg>
  )
}
